// Compute the integral of √coth(x) from 0 to 1 using the rectangle rule, the trapezoidal rule, and Simpson's rule.
...
class SqrtCoth : public Genfun::AbsFunction
{
public:
    ...
    virtual double operator()(double t) const
    {
        ...
        double x = t * t;
        return 2.0 * t * std::sqrt(std::cosh(x) / std::sinh(x));
    }

    virtual double operator()(const Genfun::Argument &t) const
    {
        return (*this)(t[0]);
    }

    virtual SqrtCoth *clone() const
    {
        return new SqrtCoth(*this);
    }
};
...
int main(int argc, char **argv)
{
    ...
    // function
    GENFUNCTION f = SqrtCoth();

    // exact solution: integral_0^1 √coth(x) dx ... mathematica
    const double exact = M_PI / 2.0 + 0.5 * std::log((std::sqrt(1.0 / std::tanh(1.0)) + 1.0) /
        (std::sqrt(1.0 / std::tanh(1.0)) - 1.0)) - std::atan(std::sqrt(1.0 / std::tanh(1.0)));
